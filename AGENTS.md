...
> เป็นภาษาอังกฤษ
>
> **ระบบต้องรองรับสองภาษา (ไทย/อังกฤษ) 100%** — ข้อความที่ผู้ใช้เห็นต้องมาจากคลังข้อความ
> ห้ามฝังในคอมโพเนนต์ และ `src/modules/**` ต้องคืน `code` + พารามิเตอร์ ไม่ใช่ประโยคสำเร็จรูป
> กติกาเต็มอยู่ที่ [docs/13-i18n.md](docs/13-i18n.md) (ยังไม่ได้ลงมือทำ — ดู STATUS)
...
**สถานะปัจจุบัน (2026-09-18): เฟส 1 MVP ใช้งานได้** — รับสัตว์, SOAP, สั่ง/จ่ายยา FEFO,
POS ใบกำกับไทย, ฝากเลี้ยง, กรูมมิ่ง, พอร์ทัลจองคิว — หน้าจอพนักงาน/พอร์ทัลเป็นภาษาไทยครบแผนก
(แถบนำทางรู้หน้าปัจจุบัน, ค้นแล้วเลือกคน/สัตว์จากรายการ, สถานะไม่โชว์รหัสอังกฤษ)
...
**ก่อนส่งงานทุกครั้ง ให้ผ่าน §9 (Definition of Done) และ §11 (ห้ามผลิต AI Slop)**
...
| นำเข้าข้อมูลตอนเปิดคลินิกใหม่, เทมเพลต CSV | [docs/11-data-import.md](docs/11-data-import.md) |
| หน้าจอตั้งค่า/หลังร้าน, แอดมินแพลตฟอร์ม, สิทธิ์ต่อหน้าจอ | [docs/12-admin-console.md](docs/12-admin-console.md) |
| รองรับสองภาษา ไทย/อังกฤษ, คลังข้อความ, เอกสารสองภาษา | [docs/13-i18n.md](docs/13-i18n.md) |
...
- [ ] ผ่านเช็กลิสต์กัน **AI Slop** ใน §11.5 — ไม่มีโค้ด/เทส/เอกสารที่เพิ่มมาโดยไม่มีคนเรียกหรือไม่มีหลักฐาน
...
## 11. ห้ามผลิต AI Slop — โค้ด/เทส/เอกสารที่ "ดูดี" แต่ไม่มีคุณค่า

**AI Slop = ผลงานที่ปริมาณมากกว่าคุณค่า** — โครงสร้างเผื่ออนาคตที่ไม่มีใครเรียก,
คอมเมนต์ที่เล่าซ้ำสิ่งที่โค้ดบอกอยู่แล้ว, เทสที่ไม่มีวันแดง, รายงานสามหน้าที่อ้างว่าเสร็จ
ทั้งที่ไม่ได้รันอะไรเลย

ในโปรเจกต์นี้มันอันตรายกว่าที่อื่น เพราะโดเมนคือ **เงินและยา** —
โค้ดที่ดูเรียบร้อยแต่ผิดเงียบ ๆ แพงกว่าโค้ดที่พังเสียงดัง

### 11.1 โค้ด

| ❌ slop | ✅ ที่ต้องทำ |
| --- | --- |
| สร้าง interface / factory / strategy / generic เผื่ออนาคต | เขียนตรง ๆ จนมีผู้ใช้จริง ≥ 2 ที่ ค่อยแยก |
| helper ที่ห่อการเรียกฟังก์ชันเดียวบรรทัดเดียว | เรียกของเดิมตรง ๆ |
| สร้างไฟล์ util ใหม่ ทั้งที่ `modules/shared/` มีอยู่แล้ว | `grep -r` ใน `src/modules/shared/` ก่อนเขียนทุกครั้ง |
| `try { … } catch { return null }` / fallback เงียบ ๆ | ปล่อยให้โยน `BusinessError` — งานเงิน/สต็อก/ยา ต้องล้มดัง ไม่ใช่ผิดเงียบ |
| `any`, `as unknown as`, `@ts-expect-error` เพื่อให้ `typecheck` ผ่าน | แก้ type ให้ตรงความจริง ถ้าแก้ไม่ได้ให้ถาม |
| null-check ในจุดที่ type ยืนยันแล้วว่าเป็นไปไม่ได้ | เชื่อ type — ถ้าไม่กล้าเชื่อ แปลว่า type ผิด ให้แก้ type |
| ใส่ค่า default มั่ว ๆ ให้ `vatRate` / `taxCode` / ราคา / หน่วยนับ | ค่าธุรกิจต้องมาจาก DB หรือผู้ใช้ — ดู §8 |
| จัดฟอร์แมต/rename ทั้งไฟล์ปนมากับงานจริง | ดิฟต้องมีแต่บรรทัดที่เกี่ยวกับงานที่ถูกสั่ง |
| `console.log` ค้าง / โค้ดที่คอมเมนต์ทิ้งไว้ | ลบ — git เก็บประวัติให้แล้ว |
| `// TODO:` ลอย ๆ ในโค้ด | เขียนลง [docs/STATUS.md](docs/STATUS.md) §2 แทน ที่นั่นมีคนอ่าน |

**คอมเมนต์ต้องตอบ "ทำไม" หรือชี้ invariant/เอกสาร ไม่ใช่เล่าบรรทัดถัดไป**
...
// ✅ มีคุณค่า — บอกเหตุผลที่โค้ดไม่ได้บอก
// ปันส่วน VAT ลงบรรทัดหลังคำนวณที่ระดับบิล เศษสตางค์ตกที่บรรทัดยอดสูงสุด (§3 ข้อ 2)
```

ตัวอย่างคอมเมนต์ที่ถือว่าถูกต้องในโปรเจกต์นี้: `src/modules/shared/money.ts`

### 11.2 เทส

- **เกณฑ์เดียวที่ใช้ตัดสิน: "ถ้าลบตรรกะบรรทัดนั้นทิ้ง เทสนี้แดงไหม"** ถ้าไม่แดง = slop ให้ลบ
- ❌ เทสที่ assert ค่าที่ mock คืนมาเอง (พิสูจน์แค่ว่า mock ทำงาน)
- ❌ `expect(x).toBeDefined()` อย่างเดียว / ชื่อเทสว่า "should work"
- ❌ เพิ่มจำนวนเทสให้ตัวเลขดูเยอะ — **1 เทส 1 พฤติกรรม** ชื่อเทสบอกกฎธุรกิจเป็นภาษาไทยได้
- ✅ เคสขอบจริงของโดเมนนี้ที่ควรมี: เศษสตางค์ปัดแล้วผลรวมไม่ตรง, VAT ปันส่วนไม่ลงตัว,
  ล็อตหมดอายุวันนี้, สต็อกพอบางส่วน, รันคิดค่าห้อง/ออกบิลซ้ำแล้วยอดไม่เพิ่ม, tenant อื่นมองไม่เห็น
- **ห้ามแก้เทสให้ผ่าน, ห้าม `.skip`, ห้ามลบเทส** เพื่อให้งานเดินต่อ — เทสที่ขวางคือเทสที่กำลังทำงาน
  ถ้าเทสผิดจริง ให้แก้ได้ แต่ต้องเขียนในรายงานว่าแก้เทสตัวไหนเพราะอะไร

### 11.3 เอกสารและข้อความรายงาน — จุดที่ slop เกิดบ่อยที่สุด

- **ทุกประโยคที่อ้างว่า "ผ่าน / เสร็จ / ใช้งานได้" ต้องมีคำสั่งที่รันจริงกำกับพร้อมผลย่อ**
  เช่น `npm test` → 18 passed ไม่มีผลรัน = ห้ามเขียนคำว่าผ่าน (ย้ำ §9)
- ❌ คำโฆษณาไร้หลักฐาน: "production-ready", "robust", "enterprise-grade",
  "ครบถ้วนสมบูรณ์", "ปรับปรุงประสิทธิภาพอย่างมาก" — มีตัวเลขให้ใส่ตัวเลข ไม่มีให้ตัดทิ้ง
- **รายงานต้องสั้นกว่างาน** ดิฟ 30 บรรทัด ไม่ต้องมีสรุปสามหน้าพร้อมตารางประดับ
- ❌ สร้างไฟล์ `.md` ใหม่เพื่อสรุปงาน (`SUMMARY.md`, `REPORT.md`, `IMPLEMENTATION_PLAN.md`)
  — ที่สรุปมีที่เดียวคือ [docs/STATUS.md](docs/STATUS.md)
- ❌ คัดลอกกติกาไปวางซ้ำในไฟล์อื่น — ชี้มาที่ `AGENTS.md` เสมอ (§4, §10)
- ใน STATUS.md ช่อง "สถานะการตรวจสอบ" ต้องแยก **"เขียนเสร็จ" ออกจาก "ทดสอบแล้ว"**
  อย่าติ๊ก ✅ รวดเดียวทั้งตาราง

### 11.4 ขอบเขตงาน

- ทำเท่าที่ถูกสั่ง — ไม่ refactor ข้างเคียง ไม่เพิ่มฟีเจอร์แถม ไม่อัปเกรด dependency ระหว่างทาง
- เจอปัญหาอื่นระหว่างทาง → บันทึกลง STATUS §2 แล้วทำงานเดิมต่อ
  (ยกเว้นมันบล็อกงานอยู่จริง → หยุดแล้วบอกผู้ใช้)
- **ไฟล์ใหม่ทุกไฟล์ต้องมีคนเรียกตั้งแต่ commit แรก** ถ้ายังไม่มีใครเรียก = ยังไม่ถึงเวลาสร้าง

### 11.5 เช็กก่อนบอกว่าเสร็จ (30 วินาที)

```bash
git diff --stat    # ดิฟใหญ่เกินงานที่ถูกสั่งไหม
git diff -U0 | grep -nE '^\+.*(console\.log|TODO|FIXME|as any|as unknown as|@ts-expect-error)'
```

แล้วตอบ 5 คำถามนี้ — **ข้อไหนตอบไม่ได้ ให้ลบส่วนนั้นทิ้งก่อนส่ง**

1. ไฟล์ / ฟังก์ชัน / พารามิเตอร์ ที่เพิ่มมา **ใครเรียก**
2. abstraction ที่เพิ่มมา มีผู้ใช้จริงกี่ที่ (ถ้า 1 → เขียนตรง ๆ)
3. คอมเมนต์ที่เพิ่มมา ตอบ "ทำไม" หรือแค่เล่าโค้ด
4. เทสที่เพิ่มมา **แดงได้จริงไหม** ถ้าตรรกะพัง
5. ทุกประโยคในรายงานที่อ้างว่าผ่าน มีผลรันจริงรองรับไหม

> **ตอนนี้เครื่องตรวจให้ได้แค่บางส่วน** — `lint` จับได้เฉพาะกฎ import ข้ามชั้น (§6)
> ส่วน `typecheck` / `test` ไม่รู้จัก slop ที่เหลือทั้งหมดเป็นความรับผิดชอบของคนเขียน
> เจอ slop ของคนก่อนหน้า (รวมถึงของ AI ตัวก่อน) ให้บันทึกไฟล์+บรรทัดไว้ใน STATUS §2
> **อย่าตามไปแก้ทันทีระหว่างทำงานอื่น** — ดิฟที่ปนกันตรวจไม่ได้
