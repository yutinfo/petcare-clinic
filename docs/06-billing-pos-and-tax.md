...
### 6.2 การจัดประเภทภาษี — ที่ตัดสินแล้ว และที่ยังค้าง

การให้บริการรักษาพยาบาลของ **สถานพยาบาลตามกฎหมายว่าด้วยสถานพยาบาล** ได้รับยกเว้น VAT
แต่กฎหมายฉบับนั้นเป็นเรื่องสถานพยาบาลสำหรับคน ส่วนสถานพยาบาลสัตว์อยู่ภายใต้กฎหมายคนละฉบับ

**✅ ตัดสินแล้ว 2026-09-18 (ผู้ใช้เป็นผู้ตอบ): บริการทางสัตวแพทย์ในระบบนี้ถือว่าเสีย VAT**
ค่าตั้งต้นของ `ServiceItem.taxCode` ทุกหมวดจึงเป็น `VAT7` ตรงกับ `@default(VAT7)` ในสคีมา

| รายการ | ค่าตั้งต้น `taxCode` | หมายเหตุ |
| --- | --- | --- |
| `ServiceItem` ทุกหมวด (CONSULT → BOARDING) | `VAT7` | ตามคำตอบข้างต้น |
| `Product` ชนิด `RETAIL` / `SUPPLEMENT` / `CONSUMABLE` | `VAT7` | ขายสินค้าทั่วไป |
| `Product` ชนิด `DRUG` / `VACCINE` / `FOOD` | `VAT7` **ชั่วคราว** | ⚠️ ยังไม่ยืนยัน — ดูข้อค้างด้านล่าง |
| ทุกรายการ เมื่อ `TaxProfile.isVatRegistered = false` | `NONVAT` | คลินิกยังไม่จด VAT → ออกได้แค่ใบเสร็จรับเงิน ไม่ใช่ใบกำกับภาษี |

**🔴 ข้อที่ยังต้องให้ผู้ทำบัญชีของคลินิกยืนยัน (Q6 ใน [STATUS §3](STATUS.md)):**
ประมวลรัษฎากร มาตรา 81(1) ยกเว้น VAT ให้การขายสินค้าบางกลุ่ม ซึ่ง**อาจ**ครอบคลุม
อาหารสัตว์ และยา/เคมีภัณฑ์สำหรับสัตว์ ถ้าเข้าข่ายจริง สินค้ากลุ่มนั้นต้องตั้งเป็น `EXEMPT`
ไม่ใช่ `VAT7` และใบกำกับใบเดียวจะมีทั้งบรรทัดที่มี VAT และไม่มี VAT ปนกัน
(ระบบรองรับแล้ว — `computeInvoiceTotals` แยกฐานตาม `taxCode` และมีเทสเคสผสมอยู่)
**ห้าม AI ตัดสินข้อนี้เอง** ([AGENTS.md §8 ข้อ 1](../AGENTS.md)) — ปล่อยค่าตั้งต้นไว้
แล้วให้ผู้ทำบัญชีปรับรายรายการตอน onboarding

ระบบยังคงออกแบบให้ **`taxCode` ตั้งได้รายบริการและรายสินค้า**
(`VAT7` / `VAT0` / `EXEMPT` / `NONVAT`) รายงานภาษีขายแยกยอดตาม `taxCode`
และเช็กลิสต์เปิดคลินิกใหม่ต้องมีขั้นตอน **"ผู้ทำบัญชีตรวจ `taxCode` ทั้งแค็ตตาล็อก"** เสมอ
